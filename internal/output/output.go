...
func Write(out io.Writer, format string, results []ChunkResult, duration float64) error {
	switch format {
	case "json":
...
func WriteText(out io.Writer, results []ChunkResult) error {
	_, err := fmt.Fprintln(out, FullText(results))
	return err
}

// WriteJSON writes JSON output
func WriteJSON(out io.Writer, results []ChunkResult, duration float64) error {
	type Output struct {
...
func WriteVTT(out io.Writer, results []ChunkResult) error {
